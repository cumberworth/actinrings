...
def calc_force_balanced_concentration(R_ring, T, kd, delta, filament_L, EI):
    return kd*(math.exp(2*math.pi*EI*filament_L*delta /
                        (R_ring**3*constants.k*T)) - 1)
...
def calc_energy_minimized_radius(T, X_C, kd, delta, filament_L, N, EI):
    max_radius = N*filament_L/(2*math.pi)
